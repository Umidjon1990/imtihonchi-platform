...
// Helper functions for backward compatibility with objectStorageUtils
...
// Wrapper functions for routes.ts
const service = new ObjectStorageService();

export async function uploadToObjectStorage(key: string, body: Buffer | Uint8Array | string, contentType?: string): Promise<string> {
  const buffer = Buffer.isBuffer(body) ? body : Buffer.from(body);
  return service.uploadFile(key, buffer, contentType);
}

export async function downloadFromObjectStorage(key: string): Promise<Buffer> {
  return service.downloadFileAsBuffer(key);
}
